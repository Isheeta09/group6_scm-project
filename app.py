...
FINANCIAL_ANALYST_CSS = """
/* Financial analyst report styles */
.analyst-summary {
    background: #F3F8FF;
...
def save_strategy_plan(strategy_text):
    current_q = f"Q{st.session_state.quarter}"
    learning_objective = get_event_for_quarter(st.session_state.quarter)["learning_objective"]

    if not strategy_text or not strategy_text.strip():
        return False

    plan_text = strategy_text.strip()
    st.session_state.current_round_rationale[current_q] = {
        "Plan": plan_text,
        "Learning objective": learning_objective,
    }

    if not any(item.get("Quarter") == current_q for item in st.session_state.strategy_memory):
        st.session_state.strategy_memory.append(
            {
                "Quarter": current_q,
                "Plan": plan_text,
                "Learning objective": learning_objective,
    ...
    st.session_state.reflection_answers.append(
        {
            "Quarter": current_q,
            "Plan": plan_text,
        }
...
def generate_ai_learning_feedback():
    ...
    try:
        ...
        current_q = f"Q{st.session_state.quarter}"
        current_rationale = st.session_state.current_round_rationale.get(current_q, {})
        current_plan = current_rationale.get("Plan", "No pre-decision strategy recorded.")
        learning_objective = get_event_for_quarter(st.session_state.quarter)["learning_objective"]

        strategy_history = "\n".join(
            [f"{item.get('Quarter')}: {item.get('Plan')}" for item in strategy_memory]
        ) or "None provided."
...
Pre-decision strategy and reasoning:
{current_plan}
...
3. Whether the pre-decision strategy and recent decisions aligned with or diverged from the chosen strategy, and how different departments contributed to the outcome.
...
section[data-testid="stSidebar"] * { color: #1C1B19 !important; }
...
/* Lock/status messages in the sidebar — light mode */
...
section[data-testid="stSidebar"] .role-card {
    background: #F3F8FF !important;
    border: 1px solid rgba(28,27,25,0.18) !important;
    border-radius: 10px;
    padding: 10px 12px;
    color: #1C1B19 !important;
    font-size: 13px;
...
/* Lock/status messages in the sidebar — dark mode */
@media (prefers-color-scheme: dark) {
    section[data-testid="stSidebar"] .lock-status-card {
        background: #F3F8FF !important;
        border: 1px solid #8BBBE8 !important;
...
with st.sidebar:
    ...
    st.text_input("Group", key="group_name")

    st.markdown("---")
    st.markdown("### AI coaching settings")
    st.text_input(
        "OpenAI API key",
        type="password",
        key="openai_api_key",
    ...
    if st.session_state.openai_enabled and not st.session_state.openai_api_key:
        st.warning("Enter your OpenAI API key to enable AI feedback and reflection.")
...
Every department decision changes your KPIs. Some decisions are multiple choice, while others require an estimation, such as determining the right stock level with sliders.
        ...
        st.markdown(f"**Learning objective:** {get_event_for_quarter(st.session_state.quarter)['learning_objective']}")
        strategy_input = st.text_area(
            "Describe your quarter strategy and the reasoning behind it, including expected trade-offs and how it supports the learning objective.",
            value=st.session_state.get("current_round_strategy", ""),
            key="current_round_strategy",
            height=220,
        )

        if st.button("💡 Save strategy plan", key="save_strategy_plan"):
            if save_strategy_plan(strategy_input):
                st.success("Strategy plan and reasoning saved for this quarter.")
            else:
                st.warning("Write a strategy plan before saving.")
...
The company currently buys many components from a low-cost overseas supplier. This keeps purchase prices low, but recent disruptions have made deliveries less reliable. A European supplier is more expensive, but offers shorter lead times and better reliability. Purchasing can also split the order between both suppliers to reduce dependency.
        ...
        st.markdown("#### Expected supplier strategy impact")
        supplier_impact_table = pd.DataFrame([
            {
                "Supplier strategy": "Single Overseas Supplier",
                "Lead Time Impact": "High",
                "Risk Impact": "High",
                "Inventory holding": "High",
            },
            {
                "Supplier strategy": "Diversified Suppliers",
                "Lead Time Impact": "Moderate",
                "Risk Impact": "Moderate",
                "Inventory holding": "Moderate",
            },
            {
                "Supplier strategy": "Resilient Regional Supplier",
                "Lead Time Impact": "Low",
                "Risk Impact": "Low",
                "Inventory holding": "Low",
            },
        ])
        st.table(supplier_impact_table)
        ...
        if st.button(
            ...
            if "Purchasing" not in st.session_state.completed_games:
                if choice == "Split purchasing between both suppliers":
                    impact = apply_kpi_change(score=8, profit=-60000, inventory=30000, service=5, sustainability=2, lead_time=-2, risk=-8)
                elif choice == "Switch to the reliable European supplier":
                    impact = apply_kpi_change(score=6, profit=-90000, inventory=-50000, service=7, sustainability=4, lead_time=-4, risk=-9)
                else:
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.openai_enabled:
        if st.button("🎓 Generate AI feedback for this quarter", key="generate_quarter_ai_feedback"):
            feedback = generate_ai_learning_feedback()
            if feedback is None:
                st.error("AI feedback unavailable. Check your OpenAI API key and connection.")
            else:
                st.session_state.ai_feedback_history.append({"Quarter": current_q, "Feedback": feedback})

        if st.session_state.ai_feedback_history:
            last_feedback = st.session_state.ai_feedback_history[-1]["Feedback"]
            st.markdown("#### Latest AI feedback")
            st.write(last_feedback)
    else:
        st.info("Enable AI coaching in the sidebar to get strategy reflection and feedback after each quarter.")
...
4. What would you change in the next quarter?
    ...
    st.markdown("---")
    st.markdown("### Download CSS files")
    download_col1, download_col2 = st.columns(2)
    download_col1.download_button(
        "Download financials.css",
        FINANCIALS_CSS,
        file_name="financials.css",
        mime="text/css",
    )
    download_col2.download_button(
        "Download financial_analyst.css",
        FINANCIAL_ANALYST_CSS,
        file_name="financial_analyst.css",
        mime="text/css",
    )
...
""", unsafe_allow_html=True)
    ...
    st.markdown("### AI learning coach")
    if st.session_state.openai_enabled:
        if st.button("🎓 Generate final AI feedback", key="generate_final_ai_feedback"):
            feedback = generate_ai_learning_feedback()
            if feedback is None:
                st.error("AI feedback unavailable. Check your OpenAI API key and connection.")
            else:
                st.session_state.ai_feedback_history.append({"Quarter": f"Q{st.session_state.quarter}", "Feedback": feedback})
        ...
        st.info("Enable AI coaching in the sidebar for final reflection and feedback.")
